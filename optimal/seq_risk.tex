...
% ---------------------------------------------------------------------------
 ...
 \begin{equation}
    {\cal A}_\psi(W_0^j) = \psi W_j, \quad  0 < \psi \le 1.
 \label{eq:Ageo}
 \end{equation}
 ...
 alpha wealth increases after a rejection, the rule ${\cal A}_\psi$ spends more
 following a rejection and then gradually -- depending on $\psi$ -- reduces the
 level of subsequent tests.
 

 To motivate other investing rules of this form, we will take a small diversion
 and offer a different motivation for the geometric rule \eqn{eq:Ageo}.  We can
 think of the geometric procedure as describing how one spends down the
 available wealth until the next rejection.  If the initial wealth is $W_0$,
 then the amount invested in the $j$th test (assuming no intervening
 test rejects) is
 \begin{equation}
    \al_j = W_0\; \psi \; (1-\psi)^{j-1} \;, \quad j = 1, 2, \ldots\;.   
 \label{eq:alj1}
 \end{equation}
 It will be useful in our computations to define an investing rule for any $x$
 rather than integers, and so consider the following alternative description of
 geometric investing.  Rather than define the investing rule using a discrete
 distribution on $j=1,\,2,\ldots$, we use a density on the interval
 $[1,\infty)$.  In place of the geometric, define the density function
 \begin{equation}
   \al_{g}(x) = c_j\,\psi\,(1-\psi)^{x-1}, \quad 1 \le x\;,
 \label{eq:alx}
 \end{equation}
 ...
 \end{displaymath}
 The share of the initial wealth bid on the $j$th subsequent test is then
 defined by the integral of $\al_g(x)$ from $x=j$ to $x=j+1$,
 \begin{equation}
    \al_j = W_0 \int_j^{j+1} \al_g(x) dx = W_0 \, \psi\; (1-\psi)^{j-1} \;,
 \label{eq:alj2}
 \end{equation}
 matching the investment given by the discrete rule \eqn{eq:alj1}.  Now define
 the tail integral 
 \begin{equation}
    W(x) = W_0 \int_x^\infty \al_g(t) dt = W_0 (1-\psi)^{x-1}\;.
 \label{eq:Wx}
 \end{equation}
 For integers $j$, $W(j)$ is the wealth remaining after $j$ tests that fail to
 reject.  This tail integral defines a function $A_g$ that determines the bid
 from the available wealth,
 \begin{equation}
   A_g(w) = \al_g(W^{-1}(w)) = w\,\psi \;,
 \label{eq:Aw}
 \end{equation}
 as in the intuitive description of geometric investing given in \eqn{eq:Ageo}.
  This construction uses the inverse of the tail wealth to determine a 'test
 index' $W^{-1}(w)$ that corresponds to the current input wealth $w$.  The rule
 then invests as if bidding down from the initial wealth.



A second type of alpha-investing
 rule employs a version the universal prior for integers defined by
 \citet{rissanen83}.  Rather than invest at a fixed rate, a universal rule
 invests a higher percentage of the available wealth in the test of $H_{j+1}$
 when the wealth $W_j$ is large than when the available wealth is small.  The
 universal rule is based upon the discrete distribution with probabilities
 \begin{equation}
