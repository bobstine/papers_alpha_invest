...
\abstract{ 
 ...
 rate of false discoveries, but little is known about how the configuration of these  
 procedures influences the risk of the resulting
 estimator.  To meet these needs, we provide a computational framework based on stochastic dynamic
 ...
 driven or derived from an oracle.  This framework allows us to contrast the
...
\centerline{
 ...
 Although the simple, two-point model \eqn{eq:muj} cannot fully characterize the feasible set, the processes that do define the boundary resemble its structure.  For example, Figure \ref{fig:simRisk} graphs a realized example of the stochastic process
