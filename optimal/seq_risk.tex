...
\abstract{ 
 ...
 should be on fit rather than hypothesis testing.  Unfortunately, little is known about
 the estimation risk produced by streaming selection and how the configuration of these
 procedures influences the risk.  To meet these needs, we provide a computational
 framework based on stochastic dynamic programming that allows fast calculation
...
geometric is the highest spender.} The graph shows the alpha levels assuming no intervening test rejects and the initial wealth $W_0$=1. }
 ...
 with the density
 \begin{displaymath}
    A_u(x) = \frac{1}{(x+1) (\log_2(x+1))^2} \;, \quad 1 < x,
 \end{displaymath}
 in place of $A_g$.  $A_u(x)$ is a continuous version of the distribution defined by the
 penultimate code of \citet{elias75}.  \citet{rissanen83} obtains the universal code as a
 refinement of the penultimate code. }
...
%--------------------------------------------------------------------------
 ...
 feasible set show the risks as the signal levels varies in the model \eqn{eq:muj}. Boundary 
...
\centerline{
 ...
 Although the simple, two-point model \eqn{eq:muj} cannot fully characterize the feasible
...
\begin{figure}
 ...
 \eqn{eq:muj}.} The increasing trend shows the accumulating risk of the testimator 
