...
% ---------------------------------------------------------------------------
 ...
 Alpha-investing rules are quite general.  The underlying theory only requires
 that the level of the test of $H_j$ is bounded by the available wealth
 $W_{j-1}$ and that the test indeed have level $\al_j$.  Otherwise, an
 alpha-investing rule is allowed to use the pattern of prior rejections as
 captured by the sequence of wealths.  We can capture this dependence by
 thinking of the investing rule ${\cal A}$ as a function of the sequence of
 prior wealths $W_0^j = \{W_0, W_1, \ldots, W_j\}$.  A rule is then a map from
 the sequence of wealths to a number bounded by the current wealth and 1:
 \begin{equation}
 ...
 A second type of alpha-investing rule employs a version the universal prior for
 integers defined by \citet{rissanen83}.  Rather than invest at a fixed rate, a
 universal rule invests a higher percentage of the available wealth in the test
 of $H_{j+1}$ when the wealth $W_j$ is large than when the available wealth is
 small.  The universal rule is based upon the discrete distribution with
 probabilities
 \begin{equation}
