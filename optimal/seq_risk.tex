...
\centerline{
 ...
 \begin{equation}
   \hat\mu(\al(\cdot),w_0) = (\hat\mu_{\al(w_0)}, \hat\mu_{\al(W_1)}, \ldots, 
                       \hat\mu_{\al(W_{p-1})})',
 \label{eq:muHatAlphaInv}
 \end{equation}
 where $\al(\cdot)$ denotes the defining investing rule, and the wealth random
 variables $W_j$ given in \eqn{eq:Wj} contribute the randomness.  Not only does
 alpha investing allow the levels to vary over the testimators, but the levels
 form a Markov process; the level of the $j$th testimator depends on whether
 prior tests reject $H_k$, $k < j$.


 The most convenient expression for the risk of $\hat\mu(\al(\cdot),w_0)$ relies
 on a recurrence.  For convenience, let
 \begin{equation}
 ...
 \begin{eqnarray}
   {\cal R}(\hat\mu(\al(\cdot),w_0),\mu_{1:p}) 
    &=& R(\hat\mu_{\al(w_0)},\mu_1)
           + \ev \sum_{j=2}^p R(\hat\mu_{\al(W_{j-1})},\mu_j)  \cr
    &=& R(\hat\mu_{\al_1},\mu_1)
           + r_{\mu_1}(\al_1)\; 
             {\cal R}(\hat\mu(\al(\cdot),w_0-\al_1+\omega),\mu_{2:p})\cr
    & & \qquad + (1-r_{\mu_1}(\al_1))\; 
             {\cal R}(\hat\mu(\al(\cdot),w_0-\al_1),\mu_{2:p}) \;,
 \label{eq:riskTestAI}
 \end{eqnarray}
...
%--------------------------------------------------------------------------
 ...
 $\hat\mu(\al(\cdot),w)$ requires a similar recursive perspective.  Suppose that
 the initial level $\al_1 = \al(w_0)=0.05$; the graph in Figure
 \ref{fig:risk}(b) shows the components of the risk of this testimator.  The
 choice of $\mu_1$ is not so simple, however, as setting $\mu_1$ to $\tilde\mu =
 \arg \max R(\hat\mu_{0.05},\mu) \approx \pm 2.16$.  Doing so ignores the payoff
 $\omega$ obtained if $H_1$ is rejected and its impact on the subsequent wealths
 $W_j$.  Although setting $\mu_1 = \tilde\mu$ maximizes the risk for the first
 test, it leaves a good chance for the estimator to gain wealth by rejecting the
 first null hypothesis.  The probability that the first test rejects is
 ...
 \begin{eqnarray*}
    \mu_1 = \arg \max_{m} & \Bigl\{ & R(\hat\mu_{\al_1},m) \Bigr. 
        + r_{m}(\al_1)\; 
             \max_{\mu_{1:p}} {\cal R}(\hat\mu(\al,w_0-\al_1+\omega),\mu_{2:p})\cr
    & & + \Bigl. (1-r_{m}(\al_1)) \max_{\mu_{1:p}} \; 
             {\cal R}(\hat\mu(\al,w_0-\al_1),\mu_{2:p}) \Bigr\} \;.
 \end{eqnarray*}
 The best choice for $\mu_1$ depends on the risk produced by subsequent
 components of the sequence of testimators.
