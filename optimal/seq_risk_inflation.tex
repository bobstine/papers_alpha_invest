...
\abstract{ 
 ...
 We describe a methodology for computing the risk inflation of a procedure for
 testing a sequence of hypotheses.  Sequential testing is commonly employed in
 large variable selection problems in which the explanatory variables are
 considered sequentially, one-at-a-time rather than in mass.  The risk inflation
 of a sequence of such tests is the worst-case ratio of the cumulative risk of
 its implied estimator to the risk obtained by an oracle.  Our examples contrast
 the risk inflation of several testing procedures that can be used in practice,
 including hard thresholding and a method based on alpha-investing.  These
 numerical results motivate several possible theorems.  For example, it appears
 that the asymptotic bound $2 \log p$ from \citet{fostergeorge94} also holds in
 the context of sequential tests.  We also see that the hardest model to
 estimate in the sense of risk inflation is the model for which the oracle risk
 is $2 \log p$.
...
% ----------------------------------------------------------------------
 ...
 follow.  This approach allows the modeler to explore infinite sequences of
